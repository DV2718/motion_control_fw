//DV, trying to add timed position cmds 5/16/14
//DV, code 3/5/14 ser in/analog in/servo pulse in, closed loop on encoder
...
static void vPositionLoopTask(void *pvParameters) {
  while(true) {
    ...
    static double traj_spA = -1350, traj_error = 0; 
    static int traj_dir = TRAJ_FWD;//BKW;//FWD;
    
    //increment trajectory position
    //POS_MIN, POS_MAX
    if(traj_dir == TRAJ_FWD) 
    { 
      traj_spA += RAMP_RATE * POSITION_PERIOD;
      if(traj_spA >= TRAJ_MAX)  traj_dir = TRAJ_BKW;
    }
    
    else if(traj_dir == TRAJ_BKW)  
    {
      traj_spA -= RAMP_RATE * POSITION_PERIOD;
      if(traj_spA <= TRAJ_MIN)  traj_dir = TRAJ_FWD;
    }    
    //if cyclone pos within safe move window from traj position
    //change postion setpoint to current trajectory position
    traj_error = abs(traj_spA - Apos_pid_in);
    Bpos_pid_sp = traj_spA;  //debugs
    
    static int counter = 0;
    //if((traj_error < TRAJ_WINDOW) && (counter == 10))
    if(traj_error < TRAJ_WINDOW)
    {
      Apos_pid_sp = constrain(traj_spA, POS_MIN, POS_MAX);
      counter  = 0;
    }
    else counter++;
//    Apos_pid_sp = traj_spA;
    #endif
